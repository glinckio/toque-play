...
export default function TeamsScreen() {
    ...
    try {
      ...
      if (createAvatarUri) {
        try {
          ...
          dialog.warning('Time criado, mas erro ao enviar brasão.');
        }
      } else {
        setTeams((prev) => [newTeam, ...prev]);
      }
      ...
      dialog.error('Não foi possível criar a equipe');
    } finally {
      setCreating(false);
    }
  };
  ...
  if (loading) {
    return (
      <View style={styles.root}>
        <HeroHeader title="EQUIPES" subtitle={`${teams.length} equipes`} watermark="TEAMS" onBack={() => rootNav.goBack()} rounded />
  ...
  return (
    <View style={styles.root}>
      <HeroHeader title="EQUIPES" subtitle={`${teams.length} equipes · ${totalMembers} atletas`} watermark="TEAMS" onBack={() => rootNav.goBack()} rounded />
...
const styles = StyleSheet.create({
  ...
  avatarPickerText: {
    fontFamily: fonts.text.regular, fontSize: 11,
