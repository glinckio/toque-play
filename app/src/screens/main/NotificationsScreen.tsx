...
export default function NotificationsScreen() {
  ...
  const handleNotificationTap = async (item: DisplayItem) => {
    ...
    const friendlyTypes = ['FRIENDLY_REQUEST', 'FRIENDLY_ACCEPTED', 'FRIENDLY_REJECTED'];
    const tournamentTypes = ['TOURNAMENT_UPDATE', 'REGISTRATION_CONFIRMED', 'BRACKET_GENERATED', 'TOURNAMENT_STARTED', 'TOURNAMENT_COMPLETED', 'REFEREE_ASSIGNED'];
    const matchTypes = ['MATCH_SET', 'MATCH_FINISH', 'MATCH_REMINDER'];

    if (friendlyTypes.includes(item.type)) {
      navigation.navigate('Friendly', { screen: 'FriendlyDetail', params: { friendlyId: ref } });
    } else if (tournamentTypes.includes(item.type)) {
      navigation.navigate('Tournament', { screen: 'TournamentDetail', params: { tournamentId: ref } });
    } else if (matchTypes.includes(item.type)) {
      navigation.navigate('Tournament', { screen: 'LiveMatch', params: { matchId: ref } });
    }
  };
