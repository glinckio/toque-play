...
export default function LiveMatchScreen() {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      dialog.error(typeof msg === 'string' ? msg : 'Não foi possível iniciar.');
    } finally {
  ...
  const handlePoint = async (team: 'A' | 'B') => {
    ...
    try {
      ...
      dialog.error(typeof msg === 'string' ? msg : 'Erro ao marcar ponto.');
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleRemovePoint = async (team: 'A' | 'B') => {
    ...
    try {
      ...
      dialog.error(typeof msg === 'string' ? msg : 'Erro ao remover ponto.');
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleFinishSet = async () => {
    ...
    try {
      ...
      dialog.error(typeof msg === 'string' ? msg : 'Erro ao finalizar set.');
    }
  };
...
const s = StyleSheet.create({
  ...
  liveText: { fontSize: 11, fontFamily: fonts.text.semiBold, color: '#E04545', letterSpacing: 1.5 },
  finishedBadge: {
    flexDirection: 'row', alignItems: 'center', gap: 4,
    backgroundColor: 'rgba(31,184,122,0.1)',
    paddingHorizontal: 8, paddingVertical: 3, borderRadius: 8,
  },
  finishedBadgeText: { fontSize: 11, fontFamily: fonts.text.semiBold, color: colors.success, letterSpacing: 1 },
  setIndicator: {
    flexDirection: 'row', alignItems: 'center', gap: 4,
    backgroundColor: colors.primaryTint,
  ...
  winnerLabel: { fontSize: 11, fontFamily: fonts.text.semiBold, color: colors.textMuted, letterSpacing: 1 },
