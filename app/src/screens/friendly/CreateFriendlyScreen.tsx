...
export default function CreateFriendlyScreen({ navigation, route }: any) {
  ...
  const handleSubmit = async () => {
    if (!selectedTeam) { dialog.warning('Selecione um time.'); return; }
    if (!modality) { dialog.warning('Selecione a modalidade.'); return; }
    if (!category) { dialog.warning('Selecione a categoria.'); return; }
    if (!date) { dialog.warning('Selecione a data.'); return; }
    if (!address.trim() && !cep) { dialog.warning('Informe o CEP ou endereço.'); return; }
    ...
    try {
      ...
      dialog.error(typeof msg === 'string' ? msg : 'Não foi possível criar o amistoso.');
    } finally {
      setSubmitting(false);
    }
  };
